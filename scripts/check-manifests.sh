#!/usr/bin/env bash
# Assert that every host manifest agrees on plugin identity.
#
# Checks name / version / description across:
#   plugin.json                      (Agent Plugins 1.0 — Cursor, Copilot)
#   .codex-plugin/plugin.json        (OpenAI Codex)
#   .claude-plugin/plugin.json       (Claude Code)
#   .claude-plugin/marketplace.json  (Claude Code marketplace entry)
# plus the install wiring each host relies on:
#   .agents/plugins/marketplace.json entry source/path/policy (Codex)
#   .claude-plugin/marketplace.json entry source/skills/strict (Claude, Copilot)
#   .codex-plugin/plugin.json required fields and skills pointer
#   skills/seldon/SKILL.md exists
...
# Fail closed: a host validator that cannot run is a failure, because a green run
# must mean every advertised check actually executed. Pass --allow-skips for local
# convenience (missing claude CLI, no jsonschema, offline) — never in release CI.
allow_skips=false
if [[ "${1:-}" == "--allow-skips" ]]; then
  # Refuse the override where a green run is treated as release evidence.
...
if entry is None:
    print(f"FAIL: .claude-plugin/marketplace.json has no entry named {name!r}")
    sys.exit(1)
...
sources = {
    ...
    ".claude-plugin/marketplace.json[seldon]": entry,
}
...
# --- Install wiring: the fields each host uses to locate and install the plugin ---
...
claude_entries = [p for p in claude_mp["plugins"] if p.get("name") == name]
if len(claude_entries) != 1:
    errors.append(f".claude-plugin/marketplace.json must have exactly one entry named {name!r} (found {len(claude_entries)})")
if entry.get("source") != "./":
    errors.append(f".claude-plugin/marketplace.json[{name}].source is {entry.get('source')!r}, expected './'")
if entry.get("skills") != ["./skills/seldon"]:
    errors.append(f".claude-plugin/marketplace.json[{name}].skills is {entry.get('skills')!r}, expected ['./skills/seldon']")
if entry.get("strict") is not False:
    errors.append(f".claude-plugin/marketplace.json[{name}].strict must be false (plugin.json declares no components)")
for comp in ("skills", "commands", "agents", "hooks", "mcpServers"):
    if comp in claude:
        errors.append(f".claude-plugin/plugin.json must not declare '{comp}' (would conflict with the marketplace entry)")
...
import os
if not os.path.isfile("skills/seldon/SKILL.md"):
    errors.append("skills/seldon/SKILL.md not found — every host discovers the skill from this path")
...
# --- Host validators, where a real one exists -------------------------------
# Claude Code: `claude plugin validate` checks .claude-plugin/{plugin,marketplace}.json.
...
# Agent Plugins 1.0 (Cursor, Copilot CLI): validate plugin.json against the published schema.
schema_url="https://agent-plugins.org/schemas/1.0.0/plugin.schema.json"
py="python3"
[[ -x skills/seldon/.venv/bin/python ]] && py="skills/seldon/.venv/bin/python"
[[ -x skills/seldon/.venv/Scripts/python.exe ]] && py="skills/seldon/.venv/Scripts/python.exe"
if "$py" -c 'import jsonschema' 2>/dev/null && schema_json="$(curl -sfL --max-time 10 "$schema_url")"; then
...
# OpenAI Codex and GitHub Copilot CLI publish no manifest schema or offline validator.
# .codex-plugin/plugin.json and .agents/plugins/marketplace.json are checked
# structurally above; installability is confirmed by `codex plugin marketplace add ./`
# and `copilot plugin install ./` respectively (see CHANGELOG "Validated on").
